...
impl StakingContract {
  ...
  pub(crate) fn internal_calculate_account_reward(&self, account: &Account) -> Balance{
    let latest_block = if self.paused == true { self.pause_in_block } else {env::block_height()};
    let block_diff = latest_block - account.last_block_balance_change;
    let emission_rate = self.config.reward_numerator as u128/ self.config.reward_denumerator as u128;
